...
def gen_android_video(pdb):
    ...
    command = ["ffmpeg", "-i", frame_source, "-c:v", "libx264", "-crf", "23",
               "-profile:v", "baseline", "-level", "3.0", "-pix_fmt", "yuv420p",
               "-c:a", "aac", "-ac", "2", "-b:a", "128k",  "-movflags", "faststart", android_filename]
    old_command = ["ffmpeg", "-i", frame_source, "-s", "352x288",
               "-b:v", "384k", "-flags", "+loop+mv4", "-cmp", "256",
               "-partitions", "+parti4x4+parti8x8+partp4x4+partp8x8",
               "-subq", "6", "-trellis", "0", "-refs", "5", "-bf",
               "0", "-coder", "0", "-me_range", "16", "-g", "250",
               "-keyint_min", "25", "-sc_threshold", "40", "-i_qfactor",
               "0.71", "-qmin", "10", "-qmax", "51", "-qdiff", "4", "-c:a",
               "aac", "-ac", "1", "-ar", "16000", "-r", "13", "-ab", "32000",
               "-aspect", "3:2", "-strict", "experimental", android_filename]
    run_command(command)
...
def gen_ios_video(pdb):
    ...
    command = ["ffmpeg", "-i", frame_source, "-c:v", "libx264", "-crf", "23",
               "-profile:v", "baseline", "-level", "3.0", "-pix_fmt", "yuv420p",
               "-c:a", "aac", "-ac", "2", "-b:a", "128k",  "-movflags", "faststart", ios_filename]
    old_command = ["ffmpeg", "-i", frame_source, "-s", "352x288", "-vcodec", "libx264",
               "-vf", "scale=2*trunc(iw/2):-2,setsar=1", "-profile:v",
               "main", "-pix_fmt", "yuv420p", "-preset", "medium", "-crf",
               "23", "-x264-params", "ref=4", "-acodec", "copy", "-movflags",
               "+faststart", ios_filename]
    run_command(command)
...
def convert_to_scn(filename):
    ...
    command = ["scntool", "--convert", filename, "--format", "scn", "--output", scn_filename]
    run_command(command)
