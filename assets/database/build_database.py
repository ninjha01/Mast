...
def backup_to_json(entries, fieldnames):
    ...
    for entry in entries[:5]:
        allergen_data = list(zip(fieldnames, entry.split(",")))
        allergen_dict = {fieldname: field for fieldname, field in allergen_data}
...
def build_entry(text):
    ...
    ## Sponsor?
    entry += build_sponsor_segment(name)
    entry += "\n"
    return entry
